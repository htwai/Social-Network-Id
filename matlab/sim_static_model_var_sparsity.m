...
DC_mask = (rand((N)^2,1) <= p_sparsity).*DC_mask_full;
DC_mask = (DC_mask>0);
...
[D,B] = solve_nsi_cvx_known(op_exp_result,BC_mask,DC_mask,N,N_s);
...
fprintf('No of stubborn agents used: %i, MC Sim no: %i, Exposed = %i \n No. of present links : %i\n MSE in D: %f, MSE in B: %f, SUPPORT Error in D: %i \n',...
    N_s,mc_sim,sum(DC_mask(:)),sum(G(:)),MSE_D(nnn,mc_sim),...
...
for iii = 1 : length(p_sparsity_choice)
